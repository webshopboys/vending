...
{if $HOOK_PAYMENT}
	<h4>{l s='Please choose the payment method you want to use to pay the amount of'}&nbsp;
	<span class="price">{convertPrice price=$total_price}</span> {l s='(tax incl.)'}</h4><br />
	{if $invoice && $invoice->eu_account}
	{else}
	<p class="warning">{l s='It does not have EU account, therefore the price is subject to tax!'}</p><br />
	{/if}
